...
for voice in ${voices[@]}; do
  for command_subdir in $command_dir/*; do
    ...
    find $command_subdir -type f -name ${voice}_in001.wav -exec rm {} \;
    echo "Generating $command_subdir/${voice}_in001.wav"
    flite -voice flitevox/$voice.flitevox -o $command_subdir/${voice}_in001.wav -t "$command"

    find $command_subdir -type f -name ${voice}_in002.wav -exec rm {} \;
    echo "Generating $command_subdir/${voice}_in002.wav"
    flite -voice flitevox/$voice.flitevox -o $command_subdir/${voice}_in002.wav -s "duration_stretch=1.25" -t "$command"

    find $command_subdir -type f -name ${voice}_in003.wav -exec rm {} \;
    echo "Generating $command_subdir/${voice}_in003.wav"
    flite -voice flitevox/$voice.flitevox -o $command_subdir/${voice}_in003.wav -s "duration_stretch=1.50" -t "$command"
  done
done
